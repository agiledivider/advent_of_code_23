...
function compareHighCard(handA: string, handB: string): number {

    let maxA = handA.split("").reduce(maxCardValue(), 0)
    let maxB = handB.split("").reduce(maxCardValue(), 0)
    return maxA - maxB
}
...
function typeofwin(hand: string) {
    let result = ""
