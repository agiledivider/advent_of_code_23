...
Card 6: 31 18 13 56 72 | 74 77 10 23 35 67 36 11`
    ...
    describe("Part 2 - single card", () => {
        test.each([
            ["Card 1: 41 48 83 86 17 | 83 86  6 31 17  9 48 53", 1],
        ])('"%s", expected: %d', (gameData: string, expected: Number) => {
                let sut = new ScratchCardSet(gameData);
                expect(sut.crazyWinningCardCount()).toBe(expected);
            }
        )
    })
...
Card 5: 87 83 26 28 32 | 88 30 70 12 93 22 82 36
Card 6: 31 18 13 56 72 | 74 77 10 23 35 67 36 11`
                let sut = new ScratchCardSet(data);
                expect(sut.crazyWinningCardCount()).toBe(30);
            }
        )

        test('demo', () => {
                let sut = new ScratchCardSet(testdata);
                expect(sut.crazyWinningCardCount()).toBe(8570000);
            }
        )
    })
